...
$stmt->close();
//If the event was shared with another user, delete the event for the other user:        
if ($shared_with_user != null){
    $stmt2 = $mysqli->prepare("DELETE from events where event_id=? and month=? and year=? and day=? and username=?");
    if(!$stmt2){
        echo json_encode(array(
            "success" => false,
            "message" => "Event could not be deleted. Try again."
            ));
        exit;
    }
    //Bind the parameter
    $stmt2->bind_param('sssss', $event_id, $month, $year, $day, $shared_with_user);
    $stmt2->execute();
    $stmt2->close();
}

//Query SQL database to delete event from the original user:
$stmt3 = $mysqli->prepare("DELETE from events where event_id=? and month=? and year=? and day=? and username=?");
...
//Bind the parameter
$stmt3->bind_param('sssss', $event_id, $month, $year, $day, $user);
$stmt3->execute();
$stmt3->close();
